...
import { useBookingStatus, useAgentBooking } from "@/lib/hooks/use-agent";
import { agentApi } from "@/lib/api/agent";
import { toast } from "sonner";
import { parseApiError } from "@/lib/api/client";

export default function AgentConfirmPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  // Fetch full details
  const { data: booking } = useAgentBooking(id);
  ...
  const handleDownload = async () => {
    ...
    setDownloading(true);
    try {
      const blob = await agentApi.downloadTicket(id);
      ...
      <div className="max-w-md mx-auto py-16 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto">
            Securing agent seats and issuing ticket with the airline. This may take up to 15 seconds. Please do not close or refresh this page.
          </p>
        </div>
    ...
    return (
      <div className="max-w-md mx-auto py-16 text-center space-y-6">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto">
            Payment was taken from your agent wallet but airline ticket issuance failed. The amount was refunded to your balance.
          </p>
        </div>
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="text-center space-y-3 py-6">
        ...
        <p className="text-slate-500 max-w-md mx-auto text-sm">
          Seats secured and ticket successfully issued. A copy of the e-ticket has been sent to client email.
        </p>
      </div>
        ...
        <CardContent className="p-6 space-y-6">
          ...
          <div className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-100">
            <div className="flex items-center gap-2 text-slate-600 text-xs">
              ...
              <span>Paid via Agent Wallet</span>
            </div>
            <div className="text-right">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
        ...
        <Link href="/agent/bookings" className="w-full sm:w-auto">
          <Button variant="outline" className="w-full">View Bookings List</Button>
        </Link>
        <Link href="/agent/bookings/new" className="w-full sm:w-auto">
